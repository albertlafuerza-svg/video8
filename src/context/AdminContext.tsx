...
export function AdminProvider({ children }: { children: React.ReactNode }) {
  ...
  const broadcastChange = (change: any) => {
    ...
    // Force immediate localStorage update for cross-tab sync
    try {
      const currentState = { ...state, ...change.data };
      localStorage.setItem('admin_system_state', JSON.stringify(currentState));
      
      // Trigger storage event manually for same-tab updates
      window.dispatchEvent(new StorageEvent('storage', {
        key: 'admin_system_state',
        newValue: JSON.stringify(currentState),
        oldValue: localStorage.getItem('admin_system_state'),
